...
export const OperationsApprovalsPage: React.FC = () => {
  ...
  // Transform data format immediately if it's cached
  const initialItems = cachedData ? cachedData.map((sub: any) => ({
    id: sub._id || sub.id,
    entityId: sub.entityId,
    action: sub.action,
    type: sub.entityType || 'deal',
    title: sub.title || 'Untitled',
    store: sub.storeName || 'Unknown Store',
    ...
    highlights: sub.dataSnapshot?.highlights || [],
    isFeatured: sub.dataSnapshot?.isFeatured || false,
    isVerified: sub.dataSnapshot?.isVerified || false,
    expiresAt: sub.dataSnapshot?.expiresAt,
    description: sub.dataSnapshot?.description,
    terms: sub.dataSnapshot?.terms,
    dataSnapshot: sub.dataSnapshot
  })) : []

  const [items, setItems] = useState<ModerationItem[]>(initialItems as unknown as ModerationItem[])
      ...
      if (Array.isArray(res)) {
        const mapped: ModerationItem[] = res.map((s: any, idx: number) => {
          ...
          return {
            ...
            approvedAt: s.approvedAt || s.reviewedAt ? new Date(s.approvedAt || s.reviewedAt).toLocaleString('en-IN', { dateStyle: 'short', timeStyle: 'short' }) : undefined,
  ...
  useEffect(() => {
    ...
    // Background polling every 15s as fallback (event-driven updates handle instant sync)
                            ...
                            {item.status === 'Approved' && (
                              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '2px' }}>
                                <span style={{
                                  ...
                                  <Check size={12} /> {item.approvedByRole || 'Live'}
                                </span>
                                <span style={{ fontSize: '0.68rem', color: '#64748b' }}>
                                  {item.approvedByName || item.approvedBy || 'Operations'}
                                </span>
                              </div>
                            )}
                ...
                <div style={{ marginTop: '20px' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                    ...
                    {previewItem.approvedBy && (
                      <div style={{ gridColumn: 'span 2', background: '#ecfdf5', border: '1px solid #a7f3d0', borderRadius: '8px', padding: '10px 14px', marginTop: '6px' }}>
                        <div style={{ fontWeight: 700, color: '#15803d', fontSize: '0.84rem' }}>
                          ✓ Approved by {previewItem.approvedByRole || 'Reviewer'}: {previewItem.approvedByName || previewItem.approvedBy}
                        </div>
                        {previewItem.approvedAt && (
                          <div style={{ fontSize: '0.74rem', color: '#166534', marginTop: '2px' }}>
                            Authorized on {previewItem.approvedAt}
                          </div>
                        )}
                      </div>
                    )}
