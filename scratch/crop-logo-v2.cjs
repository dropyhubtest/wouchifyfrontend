...
function cropPNG(inputPath, outputPath) {
  ...
  console.log(`Alpha Bounding Box: X [${minX}..${maxX}], Y [${minY}..${maxY}]`);
  ...
  console.log(`Successfully saved ${outputPath} (${outBuf.length} bytes)`);
}

const input = path.join(__dirname, '../src/assets/navbar/wouchify-logo.png');
const output = path.join(__dirname, '../src/assets/mobile/wouchify-mobile-cropped-v2.png');
